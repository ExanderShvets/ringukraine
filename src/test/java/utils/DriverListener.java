...
import static base.tests.BasicTest.BASE_URL;
import static base.tests.BasicTest.driver;

public class DriverListener implements WebDriverEventListener {
    public static ArrayList<File> animationImage = new ArrayList<>();
    static ArrayList<String> errorTrace = new ArrayList<>();
    private final int count;
    private final String color;
    private long interval;
    private String currentUrl;
    private String goToUrl;
    public static int RESPONSE_STATUS_CODE = -1;
    ...
    public void beforeClickOn(WebElement webElement, WebDriver webDriver) {
        ...
        if (webElement.getText().length() == 0) {
            ...
            LogForTest.info("Click on " + getWebElementName(webElement) + " with text \"" + webElement.getText().replaceAll("\\n", " ") + "\"");
        }
    }

    /**
     * Here added sleep for 2 seconds for prevent not correct working after appear marketing token
     *
     * @param webElement current web element used by driver in real time
     * @param webDriver  current driver instance
     */
    public void afterClickOn(WebElement webElement, WebDriver webDriver) {
        if (!driver.getCurrentUrl().split("#")[0].equals(currentUrl) &&
                !driver.getCurrentUrl().contains("syrahost") &&
                driver.getCurrentUrl().contains(BASE_URL)) {
            try {
                Thread.sleep(2000);
            } catch (InterruptedException e) {
    ...
    public void onException(Throwable throwable, WebDriver webDriver) {
        ...
        if (RESPONSE_STATUS_CODE == 404) {
            LogForTest.error("404 Status code - Page Not Found :(");
            throw new NullPointerException("404 Status code - Page Not Found :(");
        }
    }
}
